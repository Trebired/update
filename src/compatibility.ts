...
export function compatibilityKey(
  combination: CompatibilityCombination | Record<string, string>,
): string {
  ...
  .map(
    ([entity, version]) =>
    `${encodeKeyPart(entity)}=${encodeKeyPart(version)}`,
  )
  .join("~");
}
...
export function normalizeCompatibilitySet(
  ...
  const combinations = readArray(
    record,
    setAliases.combinations ?? DEFAULT_COMPATIBILITY_SET_ALIASES.combinations,
    allowAliases,
  ).map((entry) =>
    normalizeCombination(entry, combinationAliases, options, allowAliases),
  );
  const signature = readOptionalRecord(
    record,
    setAliases.signature ?? DEFAULT_COMPATIBILITY_SET_ALIASES.signature,
    allowAliases,
  );
  ...
  const normalized: CompatibilitySet = {
    ...
    recordedAt: readOptionalString(
      record,
      setAliases.recordedAt ?? DEFAULT_COMPATIBILITY_SET_ALIASES.recordedAt,
      allowAliases,
    ),
...
export function findCombination(
...
): CompatibilityCombination | null {
  const matches = set.combinations.filter((combination) =>
    Object.entries(criteria).every(
      ([entity, version]) => combination.versions[entity] === version,
    ),
  );
  ...
  if (matches.length > 1) {
    throw new Error(
      "Multiple compatibility combinations matched the criteria.",
    );
  }
...
export function isCombinationReleased(
  set: CompatibilitySet,
  combination: CompatibilityCombination | Record<string, string>,
): boolean {
...
function normalizeCombination(
  ...
  aliases: NonNullable<
  GenericManifestNormalizationOptions["fieldAliases"]
  >["combination"],
  options: GenericManifestNormalizationOptions,
  allowAliases: boolean,
): CompatibilityCombination {
  const versionsRaw = readOptionalRecord(
    input,
    aliases?.versions ?? DEFAULT_COMBINATION_ALIASES.versions,
    allowAliases,
  );
  const resourcesRaw = readOptionalRecord(
    input,
    aliases?.resources ?? DEFAULT_COMBINATION_ALIASES.resources,
    allowAliases,
  );
  ...
  : normalizeVersionMap(
    Object.fromEntries(
      Object.entries(input).filter(
        ([key, value]) => key !== "resources" && typeof value === "string",
      ),
    ),
  );
  ...
  return {
    ...
    ? normalizeResources(resourcesRaw, options, allowAliases)
    : undefined,
  };
}
...
function normalizeVersionMap(
  ...
  if (Object.keys(versions).length === 0) {
    throw new Error(
      "Compatibility combination requires at least one entity version.",
    );
  }
...
function normalizeResources(
  ...
  options: GenericManifestNormalizationOptions,
  allowAliases: boolean,
): Record<string, CompatibilityResourceDescriptor> {
  return Object.fromEntries(
    Object.entries(input).map(([resourceName, value]) => {
        if (!value || typeof value !== "object" || Array.isArray(value)) {
          throw new Error(
            `Expected resource descriptor object for ${resourceName}.`,
          );
        }
        ...
        return [
          resourceName,
          {
            version: readString(
              record,
              aliases.version ?? DEFAULT_RESOURCE_ALIASES.version,
              allowAliases,
            ),
            fileName: readString(
              record,
              aliases.fileName ?? DEFAULT_RESOURCE_ALIASES.fileName,
              allowAliases,
            ),
            checksum: normalizeChecksum(
              readRecord(
                record,
                aliases.checksum ?? DEFAULT_RESOURCE_ALIASES.checksum,
                allowAliases,
              ),
            ),
          },
...
function verifySignedCompatibilitySet(
  set: CompatibilitySet,
  verificationKeys: UpdateVerificationKeyInput[],
): void {
  if (!set.signature) {
...
function readCombinationVersions(
  combination: CompatibilityCombination | Record<string, string>,
): Record<string, string> {
  if (
    "versions"in combination &&
      combination.versions &&
      typeof combination.versions === "object"
  ) {
